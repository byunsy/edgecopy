...
class cnHMM:
    ...
    def compute_transition_p_pseudo(self, t):
        """
        ...
        all_trans_p = []
        for s_idx in range(self.num_samples):
            all_trans_p.append(self.compute_sample_transition_p_pseudo(s_idx, t))
        self.set_transition_p(np.array(all_trans_p))
    
    def compute_sample_transition_p_pseudo(self, s_idx, t):
        """
        Once in dup or del position, prob of 0.5 to remain and 0.5 go back to refcn
        """
        MAXJUMP = 2
        STARTCN = int(self.point_cn[s_idx])
        CNRANGE = range(max(STARTCN-MAXJUMP,0), min(STARTCN+MAXJUMP+1,10))
        t_probs = np.full((self.num_obsrvs, self.num_hidden, self.num_hidden), np.log(1e-20))

        for from_cn in CNRANGE:
            to_cn_range = range(max(from_cn-MAXJUMP,0), min(from_cn+MAXJUMP+1,10))
            for to_cn in to_cn_range:
                
                if from_cn == STARTCN: # when in normal state
                ...
                else: # when in dup or del state
                    if from_cn == to_cn:  # Remain in current CN state
                        t_probs[:,from_cn,to_cn] = np.log(0.5)
                    t_probs[:,from_cn,STARTCN] = np.log(0.5)
    ...
    def compute_emission_p_pseudo(self, means, cn_mat, soi=None, MIN_CN=0.05):
        """
        Without updating samples for which s is in the reference set of 
        """ 
        ...
        for s_idx in samples_to_update: 
            for obs_idx in range(self.num_obsrvs):
                ...
                for cn_idx,cn in enumerate(self.hidden):
                    ...
                    emis_matrix[s_idx, cn_idx, obs_idx] = sample_ll
    ...
    def compute_exon_CN_pseudo(self, cc, MAX_ITER, MIN_CN=0.05):
        """
        Check global LL after running viterbi on all samples 
        """
        # Initialize CN matrix based on point CN estimates
        ...
        # Initialize emission probabilities
        self.init_emission_p()
        self.compute_emission_p_pseudo(cc.means, cn_matrix)
        ...
        final_cn_matrix = cn_matrix.copy()
        ...
        for s_idx in self.data_order:
            ...
            # Update CN path of sample s_idx
            final_cn_matrix[s_idx] = np.array(list(map(int, str(hidden_path).split(','))))
            final_cn_matrix[final_cn_matrix == 0] = MIN_CN 
            
        fopt_new = fopt_exon(self, final_cn_matrix)
        
        print(f"[{self.genename}] After HMM LL:", fopt_new)
        self.set_cn_mat(final_cn_matrix)    
